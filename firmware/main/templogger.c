...
static void resolve_server_and_start_mqtt() {
    ...
    esp_mqtt_client_config_t mqtt_cfg = {
        ...
        // Đặt tên Client ID cho đỡ trùng với người khác (có thể đổi tên khác
        // tùy
        // ý)
...
static void event_handler(void *arg, esp_event_base_t base, int32_t id,
    ...
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        ...
        if (err == ESP_ERR_WIFI_SSID) {
            ESP_LOGI(
                ...
                "============================================================");
            ESP_LOGI(TAG,
                     ">> [SMARTCONFIG] NÃO TRỐNG! BẬT HỨNG PASS NGAY LẬP TỨC");
    ...
    // 2. Nhóm sự kiện của hệ thống SmartConfig (thay cho Captive Portal)
        ...
        esp_smartconfig_stop(); // Xong việc thì tắt chế độ dò tìm đi cho nhẹ
                                // RAM
    }
}
...
void app_main(void) {
    ...
    // tuyến (RF). SmartConfig cũng lưu Pass vào đây để lần sau có điện là tự
    // nhận mạng.
